...
import {
    IConfigurableUiComponents,
    IExtension,
    IConfigurableAlgorithms,
    IRestApiResponse,
    IArticle,
} from 'superdesk-api';
...
import {toElasticQuery, ISuperdeskQuery} from 'core/query-formatting';
...
export function startApp(
    ...
    const userId = '5f3e540284c9a3a91cc9f02d';
    ...
    const elasticQuery = toElasticQuery(query);
    ...
    function getItems(path: string) {
        return httpRequestJsonLocal<IRestApiResponse<IArticle>>({
            ...
            urlParams: {
                aggregations: 0,
                es_highlight: 1,
                projections: JSON.stringify(['_id', 'lock_user', 'slugline', 'state']),
                source: JSON.stringify(elasticQuery),
            },
        });
    }

    window['testFn'] = () => Promise.all([
        getItems('/archive'),
        getItems('/search')],
    ).then(([archiveResponse, searchResponse]) => {
        debugger;
        console.log(archiveResponse._items.length); // 2
        console.log(searchResponse._items.length); // 1
    });
